...
5. Calcula 4 métricas (Tone Score, Acceptance Criteria, User Story Format, Completeness)
...
def evaluate_prompt(
    ...
    try:
        ...
        # Sort by ID to ensure stable ordering matching JSONL insertion order
        ...
        for i, example in enumerate(examples[:10], 1):
            ...
            if result["answer"]:
                ...
                acceptance = evaluate_acceptance_criteria_score(
                    bug_report, user_story, reference
                )
                format_score = evaluate_user_story_format_score(
                    bug_report, user_story, reference
                )
                ...
                print(
                    f"      [{i}/{min(10, len(examples))}] Tone:{tone['score']:.2f} Accept:{acceptance['score']:.2f} Format:{format_score['score']:.2f} Complete:{completeness['score']:.2f}"
                )

        avg_tone = sum(tone_scores) / len(tone_scores) if tone_scores else 0.0
        avg_acceptance = (
            sum(acceptance_criteria_scores) / len(acceptance_criteria_scores)
            if acceptance_criteria_scores
            else 0.0
        )
...
def display_results(prompt_name: str, scores: Dict[str, float]) -> bool:
    ...
    print("\nMétricas de Avaliação (Bug to User Story):")
    print(f"  - Tone Score: {format_score(scores['tone'], threshold=0.9)}")
    print(
        f"  - Acceptance Criteria Score: {format_score(scores['acceptance_criteria'], threshold=0.9)}"
    )
    print(
        f"  - User Story Format Score: {format_score(scores['format'], threshold=0.9)}"
    )
    print(
        f"  - Completeness Score: {format_score(scores['completeness'], threshold=0.9)}"
    )
